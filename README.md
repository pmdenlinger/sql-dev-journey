# SQL Dev Journey — Legal, Compliance, and Audit–Aligned

Purpose-built SQL patterns for Microsoft 365 legal and compliance workloads, with emphasis on Microsoft Purview and Microsoft Entra integrations.

This repository demonstrates SQL techniques used to support eDiscovery, audit, records management, and identity governance reporting for law firms and legal service providers (LSPs).

---

## How to Use This Repo

Each section progresses from core SQL foundations into compliance- and audit-aligned use cases. Scripts are written with **defensibility, traceability, and performance** in mind.

---

## Folder Overview

- `01-foundations/` — Data modeling, normalization, indexing basics
- `02-performance/` — Query plans, statistics, and large-log performance
- `03-compliance-audit/` — Audit log schemas and chain-of-custody reporting
- `04-purview-entra-integration/` — eDiscovery and Entra governance reporting
- `datasets/` — Small synthetic datasets for example queries
- `docs/` — Reference notes and diagrams

---

## Audience

- Legal Service Providers (LSPs)
- Law firm litigation & compliance teams
- Microsoft Purview and Entra administrators

This repo supports Microsoft credentialed compliance work.
